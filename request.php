...
if (isset($_SESSION['id_us'])) { 
    ...
    try {
        ...
        // อัปเดตสถานะการแจ้งเตือนเมื่อมีการดูรายละเอียด
        if (isset($_GET['view_notification_id'])) {
            $notification_id = $_GET['view_notification_id'];
            $updateStmt = $conn->prepare("UPDATE project SET status_pro = 'ดูแล้ว' WHERE id_pro = :notification_id");
            $updateStmt->bindParam(':notification_id', $notification_id, PDO::PARAM_INT);
            $updateStmt->execute();
        }

        $notificationCarStmt = $conn->prepare("SELECT COUNT(*) AS totalCarNotifications FROM use_car WHERE status_car = 'รออนุมัติ'");
        $notificationCarStmt->execute();
        $totalCarNotifications = $notificationCarStmt->fetch(PDO::FETCH_ASSOC)['totalCarNotifications'];

        // Fetch projects
        $projectStmt = $conn->prepare("SELECT * FROM project");
        $projectStmt->execute();
        $projects = $projectStmt->fetchAll(PDO::FETCH_ASSOC);

        $_SESSION['projects'] = $projects;

        // นับจำนวนการแจ้งเตือนที่มีสถานะรออนุมัติ
        $notificationStmt = $conn->prepare("SELECT COUNT(*) AS totalNotifications FROM project WHERE status_pro = 'รออนุมัติ'");
        $notificationStmt->execute();
        $totalNotifications = $notificationStmt->fetch(PDO::FETCH_ASSOC)['totalNotifications'];
...
<div style="text-align:center;">
        <h1 class="h1">คำ</h1><h2 class="h2">ร้องขอ</h2>
    </div>
    ...
    <div class="d-flex flex-row flex-shrink-0">
        ...
        <div class="sidebar-column">
            <ul class="nav nav-pills nav-flush flex-column mb-auto text-center">
                <li class="border-3 border-primary rounded-3">
                    <a href="requestpro.php" class="nav-link py-3 border-bottom rounded-0">
                ...
                <li class="border-3 border-primary rounded-3">
                    <a href="requestcar.php" class="nav-link py-3 border-bottom rounded-0">
                        <svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" fill="currentColor" class="bi bi-car-front" viewBox="0 0 16 16">
                            ...
                            <?php if ($totalCarNotifications > 0): ?>
                                <span class="notification-count"><?php echo $totalCarNotifications; ?></span>
                            <?php endif; ?>
                        </svg><br>
        ...
        $(document).ready(function() {
            ...
            $('.main-column').load('requestpro.php');

            // When clicking on the "คำร้องขอโครงการ" link
            $('a[href="requestpro.php"]').click(function(e) {
                e.preventDefault();

                if (window.matchMedia("(max-width: 768px)").matches) {
                    // Redirect to a different page for smaller screens
                    window.location.href = 'mobile_requestpro.php'; 
                } else {
                    // Load content on the same page for larger screens
                    $('.main-column').load('requestpro.php');
                }
            });

            // When clicking on the "คำร้องขออนุญาตไปราชการและขอใช้รถราชการ" link
            $('a[href="requestcar.php"]').click(function(e) {
                e.preventDefault();

                if (window.matchMedia("(max-width: 768px)").matches) {
                    // Redirect to a different page for smaller screens
                    window.location.href = 'mobile_requestcar.php'; 
                } else {
                    // Load content on the same page for larger screens
                    $('.main-column').load('requestcar.php');
                }
            });
        });
